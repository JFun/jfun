...
```js
import { makeRNG, fullLines, bfsSolve } from "@studio/web-game-core";
// or browser global `WebGameCore`
```

- **`makeRNG(seed)`** — canonical mulberry32. Same seed → identical stream
  everywhere (pairs with `@studio/growth-loop`'s `seedForDate`). Pinned by a golden.
