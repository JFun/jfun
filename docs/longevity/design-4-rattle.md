...
| T3 | 27–38 | **shell** (bead in a wooden crate) | callback: an adjacent SAME-COLOUR pop cracks the shell → normal bead (bomb blasts crack colour-blind) | read the crate's colour; work that colour to it |
...
>
> **3-tier labeled hard cadence (2026-07-18, after the flow/Royal-Match research):** predictable, telegraphed beats — **HARD** (~65% first-try, amber badge) at every X4, **SUPER HARD** (~55%, coral) at every X9, **EXTREMELY HARD** (~45%, violet) at L39/69/99 + the finale L106; first labeled beat at L24; a breather is guaranteed right after every X9; hard levels are NEVER adjacent (enforced by `assertCadence` in the generator). Beats get intrinsically harder specs (bigger objective need + scarcer colour + denser stone/balloon — never more bombs [player power] or shells [par wash-out]). The tier is baked into each level as `hard:"hard"|"super"|"extreme"` and surfaced in-game on the level banner + a persistent tinted HUD tag. Shell rule addendum: a crate only cracks from a SAME-COLOUR adjacent pop (bomb blasts stay colour-blind); a colour-exhausted crate becomes crackable by any pop (mercy); tapping a crate is a locked no-op wobble; remaining crates get a gold spotlight once they are the last objective.
...
- **Elements = one rule each, verified in `apps/rattle/scripts/dev/`.** stone (density×3, colourless) / shell (same-colour adjacent-pop crack; bombs crack colour-blind — Qi 2026-07-17, matches the colour the crate shows) / balloon / ice (low friction, slump) / tar (high friction, dam) / bomb (radial destroy). Certifier is beam+greedy+reachability; the 106-level campaign is generated by `gen-campaign.cjs` (author-by-intent + sawtooth).
