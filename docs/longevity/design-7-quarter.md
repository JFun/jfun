...
**Status:** design-only, no prototype yet — but the base engine is the **shipped Tilt app** (App Store, v1.0): marble physics, circle-vs-grid collision, settle detection, flood-fill seal detector, level-select/medal/Firebase/iPad shell all exist and get forked, not rebuilt. Positioned as a true App Store sequel ("Tilt 2"): same marble-and-hole soul and zero-word legibility, accelerometer swapped for a discrete rotation verb.
