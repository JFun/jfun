...
// Same approach as apps/moraine/scripts/dev/shots.cjs (web/Capacitor stack).
...
(async () => {
  const chrome = spawn(CHROME, ['--headless=new', '--disable-gpu', '--hide-scrollbars',
    '--no-first-run', '--no-default-browser-check', '--user-data-dir=/tmp/tilt-cdp-profile',
  ...
  for (const [slot, w, h, dsr] of SIZES) {
    ...
    for (const [name, q] of SHOTS) {
      ...
      await sleep(1800);   // settle level build + card animations + the harness resize passes
