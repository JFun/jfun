#!/usr/bin/env bash
# One-shot iPhone deploy (Capacitor): self-test → cap sync → cache-bust → build →
# install → launch. Wireless via devicectl (device paired with "Connect via
# network"). Canonical shape from Lanthorn/Tilt.
#
